...
const MAX_LOADED_CANVAS_COUNT = 124
...
class CanvasData:
	...
	func _notification(what: int) -> void:
		if what == NOTIFICATION_PREDELETE:
			if _canvas_load_thread.is_started():
				_canvas_load_thread.wait_to_finish.call_deferred()
...
func _on_canvas_load_state_changed(canvas_load_state : int, coords : Vector2i) -> void:
	if canvas_load_state == CanvasData.CanvasLoadingState.LOADED:
		canvas_finished_loading.emit.call_deferred(coords)
...
func set_canvas_data_image(coords : Vector2i, image : Image) -> void:
	...
	if image == null:
		canvases_by_coords[coords]._canvas_image = null
		canvases_by_coords[coords]._canvas_texture = null
		return
